...
void printUsage() {
  ...
  printf(" -k startingKeyIdx: Specify the index of the key to start the search from\n");
...
int main(int argc, char* argv[]) {
  ...
  while (a < argc) {
    ...
    if(strcmp(argv[a], "-t") == 0) {
      ...
      CHECKARG("-k",1);
      startingKeyIdx = getInt("startingKeyIdx",argv[a]);
      a++;
    } else if(a == argc - 1) {
  ...
  Kangaroo *v = new Kangaroo(secp,dp,gpuEnable,workFile,iWorkFile,savePeriod,saveKangaroo,saveKangarooByServer,
                             maxStep,wtimeout,port,ntimeout,serverIP,outputFile,splitWorkFile,startingKeyIdx);
  if(checkFlag) {
